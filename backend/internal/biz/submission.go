...
func validateSubmissionVerificationCode(task *data.Task, sub *data.Submission, actingUserID string) error {
	...
	if task.CanManage(actingUserID) {
		return nil
	}
...
func (uc *SubmissionUsecase) CreateSubmission(ctx context.Context, sub *data.Submission) error {
	...
	// 普通用户限制唯一提交，任务管理者和协作者可多次提交。
	if !task.AllowsMultipleSubmissions(sub.UserID) {
...
func (uc *SubmissionUsecase) UpdateSubmission(ctx context.Context, id string, userID string, sub *data.Submission) error {
	...
	if existing.UserID != userID && !task.CanManage(userID) {
...
func (uc *SubmissionUsecase) GetSubmission(ctx context.Context, id string, userID string) (*data.Submission, error) {
	...
	if submission.UserID != userID && !task.CanManage(userID) {
...
func (uc *SubmissionUsecase) GetSubmissionForTaskCreator(ctx context.Context, id string, userID string) (*data.Submission, *data.Task, error) {
	...
	if !task.CanManage(userID) {
		return nil, nil, errors.New("只有任务创建者或管理员可以操作")
	}
...
func (uc *SubmissionUsecase) DeleteSubmission(ctx context.Context, id string, userID string) error {
	...
	if !task.CanManage(userID) && submission.UserID != userID {
		return errors.New("无权限删除此提交")
	}
